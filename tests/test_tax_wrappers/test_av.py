...
def test_av_before_min_holding():
    ...
    with (
        ...
        withdrawal = Withdrawal(pct=1.0, datetime=(opening + av.TAX_MINIMAL_HOLDING_PERIOD).subtract(days=1))
...
@pytest.mark.parametrize(
    ...
    argvalues=[
        ...
        (200_000, 40_000, 40_000 * PS + max(0, 40_000 - AV.TAX_RELIEF) * (AV.PF * AV.SOFT_CONTRIBUTION_LIMIT + (200_000 - AV.SOFT_CONTRIBUTION_LIMIT) * ISR) / 200_000),
    ]
)
def test_pea_after_min_holding(contribution: float, gains: float, expected_tax: float):
    ...
    with (
        ...
        withdrawal = Withdrawal(pct=1.0, datetime=(opening + av.TAX_MINIMAL_HOLDING_PERIOD).add(days=1))
