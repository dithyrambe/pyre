...
class TaxWrapper(ABC):
    ...
    def __init__(self, fees: float = 0.0, annualized_return: float = 0.0, opening_date: DateTime | None = None) -> None:
    ...
    def total_contribution(self, datetime: DateTime):
        return sum(_.amount for _ in filter(lambda contrib: contrib.datetime <= datetime, self.contributions))

    def portfolio_value(self, datetime: DateTime):
        months_since_contributions = [(datetime - contrib.datetime).in_months() for contrib in self.contributions]
    ...
    def contribute(self, contribution: Contribution) -> None:
        ...
        if self.total_contribution(contribution.datetime) + contribution.amount > self.CONTRIBUTION_LIMIT:
